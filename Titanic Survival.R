...
library(readr) # for reading the loaded csv files
#load data
train <- read_csv("~/titanic/train_titanic.txt")
test <- read_csv("~/titanic/test_titanic.txt")
View(test)
View(train)
...
full$Famil <- paste(full$Surname, full$Fsize, sep=' ')
full$Famil
...
#We can now  see that there's a survival penalty to singletons and those with #family sizes above 4. We can collapse this variable into three levels which #will be helpful since there are comparatively fewer large families. Let's #create a **discretized family size** variable.
